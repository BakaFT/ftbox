    ...
    unsigned char maxBackupFiles)
{
    ...
    /* setvbuf(s_flog.output, NULL, _IONBF, 0); */
    /* setbuf(s_flog.output, NULL);*/
...
static void getTimestamp(const struct timeval* time,
    char* timestamp,
    size_t size)
{
    time_t
        sec
        = time->tv_sec; /* a necessary variable to avoid a runtime error on
                             Windows */
    struct tm calendar;
...
static void getBackupFileName(const char* basename,
    unsigned char index,
    char* backupname,
    size_t size)
{
    ...
    unsigned long long* flushedTime)
{
    ...
    if ((size = fprintf(fp, "%c %s %ld %s:%d: ", levelc, timestamp, threadID,
             file, line))
        > 0) {
        totalsize += size;
    }
    ...
    ...)
{
    ...
    if (hasFlag(s_logger, kConsoleLogger)) {
        va_start(carg, fmt);
        vflog(s_clog.output, levelc, timestamp, threadID, file, line, fmt, carg,
            currentTime, &s_clog.flushedTime);
        va_end(carg);
    }
    if (hasFlag(s_logger, kFileLogger)) {
        if (rotateLogFiles()) {
            va_start(farg, fmt);
            s_flog.currentFileSize += vflog(s_flog.output, levelc, timestamp, threadID, file, line, fmt,
                farg, currentTime, &s_flog.flushedTime);
            va_end(farg);
        }
    }
    unlock();
